...
class Zleft(Piece):
    """Define Z left piece: (*) primary_shape | (¤) secondary_shape
       UP/DOWN    RIGHT/LEFT
       * *          *
         ¤ ¤      ¤ *
                  ¤
    ...
    def __init__(self, surface, gameboard):
        super(Zleft, self).__init__(surface, gameboard, color=PINK)
        self.width = int(60)
        self.height = int(40)
        self.can_rotate = True

    def set_position(self, x, y):
        super(Zleft, self).set_position(x, y)
        height = self.height
        width = self.width
        ...
        if self.orientation == "up" or self.orientation == "down":
            ...
            self.secondary_shape.x = x + (width/3)
